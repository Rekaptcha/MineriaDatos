...
**Minería de datos:**
- La minería de datos trabaja con datos heterogéneos (datos que pueden ser vídeos, imágenes..) y abundantes (muchas observaciones y/o varibles) así como muchas veces, trabaja con datos de mala calidad (decimos que un conjunto de datos es de mala calidad cuando presenta muchos datos atípicos y/o faltantes, por ese motivo no se van a poder usar las técnicas que se usan en estadística clásica), mientras que la estadística trabaja con datos de cierta calidad. En ocasiones la recogida de datos de minería de datos se realiza de forma automática, no existiendo un control que corrija los datos faltantes.

- Pone más énfasis en la predicción que en la explicación. Interesa más predecir qué pasará a partir de los datos analizados que explicar por qué se producen esos resultados. Eso permite aplicar modelos más complejos y obtener resultados más precisos, perdiendo capacidad para explicar los resultados. (Ejemplo: nos interesa saber si un individuo va a adquirir o no un producto pero nos da igual si se debe a la VI1, VI2 o VI3, sexo, edad o poder adquisitivo). Por ello la mayoría de modelos que se van a ver son modelos de caja negra, en los cuales se obtienen buenos resultados de predicción pero no se puede saber exactamente a cuáles de las vv se debe esos valores de predicción.

- El tamaño de muestra al ser tan grande hace imposible la aplicación de inferencia (sacar conclusiones de un conjunto de población a la cual no podemos acceder a partir de los valores de una muestra). Se trabaja con muchos datos, y el gran tamaño de la muestra precisamente suele hacer imposible la aplicación de inferencia en la predicción por la potencia de los contrastes (si la potencia de un contraste de hipótesis es demasiado alta, como así ocurre con conjuntos de datos muy grandes), siempre se rechazarán las hipótesis nulas, lo que inutiliza este tipo de técnica. Necesitamos un plan B, y ahí surge a nuestro rescate la partición de los datos (es lo que habitualmente se usa en minería de datos).
...
//Básicamente se divide el conjunto de datos en dos, dividimos la muestra en dos partes, simulando que desconocemos la info del subgrupo ''de prueba'', sacamos conclusiones del subgrupo de entrenamiento y verificaremos si estas conclusiones son aplicables también al conjunto de datos de prueba. Si es así, cabe esperar que podamos aplicar dichas conclusiones a cualquier conjunto de datos que venga en el futuro. Por tanto:  

-El conjunto de datos de entrenamiento (training) sirve para generar y definir el modelo.  
-El conjunto de datos de prueba (test) permite evaluar la bondad del modelo obtenido (verificar si se puede llevar a cabo la generalización o no).  
